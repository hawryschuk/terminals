...
export const testTerminal = (t: Promise<Terminal>) => {
    let terminal!: Terminal;

    before(async () => terminal = await t);
    ...
    it('allows responding to simultaneous prompts', async () => {
        ...
        await Util.waitUntil(() => Object.keys(terminal.prompts).length === 3 && !Object.keys(terminal.prompts).includes('age'));
